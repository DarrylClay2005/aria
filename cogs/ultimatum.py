...
class DilemmaView(discord.ui.View):
    ...
    async def handle_choice(self, interaction: discord.Interaction, choice: str):
        ...
        if self.choices[interaction.user.id] is not None:
            return await interaction.response.send_message("Choice locked.", ephemeral=True)
        self.choices[interaction.user.id] = choice
        await interaction.response.send_message(f"You chose to **{choice}**.", ephemeral=True)
        if self.choices[self.user_a.id] is not None and self.choices[self.user_b.id] is not None:
            self.stop()

    @discord.ui.button(label="Cooperate (Trust)", style=discord.ButtonStyle.green, emoji="🤝")
    async def btn_cooperate(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self.handle_choice(interaction, "cooperate")

    @discord.ui.button(label="Betray (Damage Mind)", style=discord.ButtonStyle.red, emoji="🔪")
    async def btn_betray(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self.handle_choice(interaction, "betray")
...
class Ultimatum(commands.Cog):
    ...
    async def trolley_problem(self, interaction: discord.Interaction, user_a: discord.Member, user_b: discord.Member):
        ...
        prompt = (
            f"Forcing {user_a.display_name} and {user_b.display_name} into a Prisoner's Dilemma. "
            "Coop/Coop = +10 Sanity. Betray = Steal 30 Sanity, Victim loses 50 Sanity. "
            "Double Betray = Both lose 40 Sanity. Introduce this game cynically."
        )
        try:
            ...
            intro_text = f"{user_a.mention} {user_b.mention}. Betray or Cooperate. Your minds are on the line."
        ...
        if c_a is None or c_b is None:
            return await interaction.channel.send("Experiment canceled. Someone didn't answer.")
