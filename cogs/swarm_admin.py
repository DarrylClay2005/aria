...
SWARM_ENV_PREFIX = "ARIA_SWARM"
DB_CONFIG = {
    'host': os.getenv(f"{SWARM_ENV_PREFIX}_DB_HOST", "127.0.0.1"),
    'user': os.getenv(f"{SWARM_ENV_PREFIX}_DB_USER", "botuser"),
    'password': os.getenv(f"{SWARM_ENV_PREFIX}_DB_PASSWORD", ""),
    'db': os.getenv(f"{SWARM_ENV_PREFIX}_DB_NAME", "discord_music_gws"),
    'autocommit': True
}
WEBHOOK_URL = os.getenv(f"{SWARM_ENV_PREFIX}_WEBHOOK_URL", "").strip()

def validate_swarm_admin_config():
    required = {f"{SWARM_ENV_PREFIX}_DB_PASSWORD": DB_CONFIG.get("password")}
    missing = [name for name, value in required.items() if not value]
    if missing:
        raise RuntimeError(f"Missing required environment variables: {', '.join(missing)}")

validate_swarm_admin_config()
...
class SwarmAdmin(commands.Cog):
    ...
    async def medic_task(self):
        # 5. The Medic (Auto-Failover detection with Cross-DB Routing)
        try:
            ...
            async with DBPoolManager() as pool:
                async with pool.acquire() as conn:
                    async with conn.cursor(aiomysql.DictCursor) as cur:
                        for b in DRONE_NAMES:
                            try:
                                await cur.execute(f"SELECT title FROM discord_music_{b}.swarm_analytics WHERE guild_id = %s", (interaction.guild_id,))
                    ...
                    async with conn.cursor(aiomysql.DictCursor) as cur:
                        for d in DRONE_NAMES:
                            try:
                                ...
                                await cur.execute(f"SELECT guild_id, is_playing FROM discord_music_{d}.{d}_playback_state LIMIT 1")
                                state = await cur.fetchone()
                                if state:
                                    active_nodes += 1
                                    gid = state['guild_id']
                                    is_p = state['is_playing']
                                    await cur.execute(f"SELECT title FROM discord_music_{d}.{d}_queue ORDER BY id ASC LIMIT 1")
                                    q_top = await cur.fetchone()
                                    track = q_top['title'] if q_top else "Unknown Track"
                                    await cur.execute(f"SELECT COUNT(*) as q_len FROM discord_music_{d}.{d}_queue")
                    ...
                    async with conn.cursor() as cur:
                        for b in bots:
                            try:
                                ...
                                await cur.execute(f"TRUNCATE TABLE discord_music_{b}.{b}_queue")
                            except:
                                pass
        ...
        try:
            async with DBPoolManager() as pool:
                async with pool.acquire() as conn:
                    async with conn.cursor() as cur:
                        for b in bots:
                            await cur.execute(f"CREATE TABLE IF NOT EXISTS discord_music_{b}.{b}_guild_settings (guild_id BIGINT PRIMARY KEY, loop_mode VARCHAR(10) DEFAULT 'off', volume INT DEFAULT 100, audio_filter VARCHAR(20) DEFAULT 'none')")
            ...
            async with DBPoolManager() as pool:
                async with pool.acquire() as conn:
                    async with conn.cursor() as cur:
                        for b in bots:
                            await cur.execute(f"CREATE TABLE IF NOT EXISTS discord_music_{b}.{b}_guild_settings (guild_id BIGINT PRIMARY KEY, loop_mode VARCHAR(10) DEFAULT 'off', volume INT DEFAULT 100, audio_filter VARCHAR(20) DEFAULT 'none')")
                            try: await cur.execute(f"ALTER TABLE discord_music_{b}.{b}_guild_settings ADD COLUMN audio_filter VARCHAR(20) DEFAULT 'none'")
                            except: pass
                            await cur.execute(f"INSERT INTO discord_music_{b}.{b}_guild_settings (guild_id, audio_filter) VALUES (%s, %s) ON DUPLICATE KEY UPDATE audio_filter = %s", (interaction.guild_id, filter_type.value, filter_type.value))
            msg = f"🎛️ **Audio Filter:** Set to `{filter_type.name}` for `{drone.name}`. Live detection will apply it momentarily." if drone else f"☢️ **GLOBAL FILTER:** Set to `{filter_type.name}` for ALL nodes."
            await interaction.followup.send(msg)
        except Exception as e: await interaction.followup.send(f"❌ Error: {e}")
