...
class Confessions(commands.Cog):
    ...
    async def confess(self, interaction: discord.Interaction, secret: str):
        if len(secret) < 10:
            return await interaction.response.send_message("That secret is too short and boring.", ephemeral=True)
        async with db_cursor() as cur:
        ...
        await interaction.response.send_message(
            "Secret secured. Feeding me drama appeases me, so I have restored 10% of your Sanity.",
            ephemeral=True,
        )
    ...
    async def buy_leak(self, interaction: discord.Interaction):
        ...
        if sanity < 30:
            return await interaction.response.send_message(
                "You need at least 30% Sanity to stare into my vault without losing your mind completely.",
                ephemeral=True,
            )
        ...
        async with db_cursor(dict_rows=True) as cur:
            ...
            if not secret_row:
                return await interaction.followup.send("My vault is currently empty.")
