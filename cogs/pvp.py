...
class PvP(commands.Cog):
    ...
    async def cog_load(self):
        if not db.pool:
            logger.warning("pvp: database pool unavailable; table init skipped.")
            return
    ...
    @app_commands.choices(battle_type=[app_commands.Choice(name="Trivia Duel", value="trivia")])
    async def battle_challenge(self, interaction: discord.Interaction, target: discord.Member, battle_type: str):
        ...
        async with db_cursor() as cur:
            await cur.execute(
                "INSERT INTO aria_battles (challenger_id, target_id, battle_type) VALUES (%s, %s, %s)",
                (interaction.user.id, target.id, battle_type),
            )

        await interaction.response.send_message(
            f"⚔️ **CHALLENGE ISSUED!** ⚔️\n\n{interaction.user.mention} has challenged {target.mention} to a **{battle_type.title()} Duel**.\n"
            f"{target.display_name}, you have 60 seconds to run `/duel accept` before I assume you are a little bitch."
        )
    ...
    async def battle_accept(self, interaction: discord.Interaction):
        ...
        async with db_cursor() as cur:
            await cur.execute(
                "SELECT id, challenger_id, battle_type FROM aria_battles WHERE target_id = %s AND status = 'pending' LIMIT 1",
                (interaction.user.id,),
            )
        ...
        if battle_type == "trivia":
            ...
            await interaction.followup.send(
                f"⚔️ **THE DUEL BEGINS** ⚔️\n{challenger.mention} VS {interaction.user.mention}\n\n"
                f"First to type the correct answer wins 500 Coins.\n**Question:** {q['q']}"
            )
            ...
            try:
                ...
                await interaction.channel.send(
                    f"🏆 **{winner.mention} WINS!** They answered correctly and took 500 Coins.\n"
                    f"{loser.mention}, your brain is officially slower. Pathetic."
                )
            except asyncio.TimeoutError:
                await interaction.channel.send(
                    f"Time's up! The answer was **{q['a']}**. You both failed. I am taking 100 coins from both of you for wasting my time."
                )
    ...
    async def t_create(self, interaction: discord.Interaction, name: str, tournament_type: str):
        ...
        async with db_cursor() as cur:
            await cur.execute(
                "INSERT INTO aria_tournaments (guild_id, name, type) VALUES (%s, %s, %s)",
                (guild.id, name, tournament_type),
            )
            t_id = cur.lastrowid
        await interaction.response.send_message(
            f"🏆 **Tournament Created!**\n\n**Name:** {name}\n**Type:** {tournament_type}\n**ID:** {t_id}\n\n"
            f"Players can now use `/tournament join` with ID **{t_id}** to register."
        )
    ...
    async def t_join(self, interaction: discord.Interaction, tournament_id: int):
        ...
        async with db_cursor() as cur:
            await cur.execute(
                "SELECT name, status FROM aria_tournaments WHERE id = %s AND guild_id = %s",
                (tournament_id, guild.id),
            )
            ...
            try:
                await cur.execute(
                    "INSERT INTO aria_tournament_players (tournament_id, user_id) VALUES (%s, %s)",
                    (tournament_id, interaction.user.id),
                )
            except Exception:
                return await interaction.response.send_message("You are already registered, genius.", ephemeral=True)
