...
class Pacts(commands.Cog):
        ...
        async with aiomysql.create_pool(**DB_CONFIG) as pool:
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    if res and res[0]:
                        # Drain 1 Sanity per message
                        await cur.execute("UPDATE aria_sanity SET sanity_level = GREATEST(0, sanity_level - 1) WHERE user_id = %s", (message.author.id,))
                        ...
                        if sanity == 0:
                            ...
                            await message.channel.send(f"⚠️ **PSYCHOLOGICAL BREAK.** {message.author.mention}'s sanity has hit 0%. The pact has devoured their mind. I am revoking my favor and placing them in an asylum (timeout).")
                            try:
        ...
        async with aiomysql.create_pool(**DB_CONFIG) as pool:
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    res = await cur.fetchone()
                    if res and res[0]:
                        return await interaction.response.send_message("You already sold your mind to me. You can't sell it twice.", ephemeral=True)
                    ...
                    await cur.execute("INSERT INTO aria_pacts (user_id, pact_active) VALUES (%s, TRUE) ON DUPLICATE KEY UPDATE pact_active = TRUE", (interaction.user.id,))
    ...
    async def break_pact(self, interaction: discord.Interaction):
        ...
        # FIXED: Synchronous Gemini call is now properly awaited in an executor to prevent bot freeze
        try:
        ...
        await interaction.followup.send(f"Want out? Answer this riddle in 30 seconds. Fail, and I instantly drain 50% of your Sanity.\n\n📜 **The Riddle:**\n*{riddle}*")

        def check(m): return m.channel == interaction.channel and m.author.id == interaction.user.id
        ...
        try:
            ...
            async with aiomysql.create_pool(**DB_CONFIG) as pool:
                async with pool.acquire() as conn:
                    async with conn.cursor() as cur:
                        await cur.execute("UPDATE aria_sanity SET sanity_level = GREATEST(0, sanity_level - 50) WHERE user_id = %s", (interaction.user.id,))
            await interaction.channel.send(f"Wrong. The answer was **{correct_answer}**. I just ripped 50% of your sanity away, and I still own your mind.")
