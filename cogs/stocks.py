            ...
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    for symbol, current_price in stocks:
                        # Stocks can drop by up to 25% or rise by up to 30%
                        change_percent = random.uniform(-0.25, 0.30)
                        new_price = max(5, int(current_price + (current_price * change_percent))) # Minimum price is 5
        ...
        async with aiomysql.create_pool(**DB_CONFIG) as pool:
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    await self.update_balance(interaction.user.id, -total_cost)
        ...
        async with aiomysql.create_pool(**DB_CONFIG) as pool:
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    await self.update_balance(interaction.user.id, total_gained)
