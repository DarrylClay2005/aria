...
class BookClub(commands.Cog):
    ...
    async def start_club(self, interaction: discord.Interaction):
        ...
        async with db_cursor() as cur:
            ...
            await cur.execute(
                "INSERT INTO aria_bookclub (title, mal_url, image_url) VALUES (%s, %s, %s)",
                (manga['title'], manga['url'], manga['images']['jpg']['large_image_url']),
            )

        embed = discord.Embed(
            title=f"📖 MANDATORY READING: {manga['title']}",
            url=manga['url'],
            description=(
                "You people are culturally bankrupt.\n\n"
                "**ASSIGNMENT:** Submit a review using `/bookclub review`. Failure results in massive Sanity damage."
            ),
    ...
    async def review(self, interaction: discord.Interaction, your_review: str):
        async with db_cursor() as cur:
            ...
            if not await cur.fetchone():
                return await interaction.response.send_message("No active book club.", ephemeral=True)
            await cur.execute(
                "INSERT INTO aria_bookclub_reviews (user_id, review) VALUES (%s, %s) ON DUPLICATE KEY UPDATE review = %s",
                (interaction.user.id, your_review, your_review),
            )
    ...
    async def conclude_club(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=False)
        async with db_cursor(dict_rows=True) as cur:
            await cur.execute("SELECT title FROM aria_bookclub WHERE active = TRUE")
            book = await cur.fetchone()
            if not book:
                return await interaction.followup.send("No active book club.")
            ...
            for slacker in slackers:
                await self.alter_sanity(slacker, -30)
            await cur.execute("UPDATE aria_bookclub SET active = FALSE")

        if not reviews:
            return await interaction.followup.send("Nobody submitted a review. Everyone loses 30% Sanity.")
        ...
        try:
            ...
            embed = discord.Embed(
                title=f"📚 Book Club Conclusion: {book['title']}",
                description=res.text[:4000] + reward_text,
                color=discord.Color.red(),
            )
            if slackers:
                embed.add_field(
                    name="🗑️ Uncultured Swine (Suffered 30% Sanity Damage)",
                    value=" ".join([f"<@{s}>" for s in slackers[:15]]),
                    inline=False,
                )
            await interaction.followup.send(embed=embed)
        except Exception:
            await interaction.followup.send("Your reviews crashed my AI.")
