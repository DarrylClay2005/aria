...
class AutonomousEngine:
    def __init__(self, bot):
        ...
        self._bot_schema_autofix_enabled = str(os.getenv("ARIA_AUTONOMY_BOT_SCHEMA_AUTOFIX", "0") or "0").strip().lower() in {"1", "true", "yes", "on"}
        self._bot_schema_autofix_notice: set[str] = set()
        ...
        for task in tasks:
            ...
            if resolved:
                async with db.pool.acquire() as conn:
                    async with conn.cursor() as cur:
                        try:
                            # Guard against uniq_open_task duplicate: a resolved row for this
                            # signature may already exist if the task was recycled by ON DUPLICATE
                            # KEY UPDATE in _schedule_repair_followup. Only update rows that are
                            # not already resolved to avoid hitting the unique constraint.
                            await cur.execute(
                                "UPDATE aria_repair_tasks SET status='resolved', last_result=%s WHERE id=%s AND status != 'resolved'",
                                ('verified by follow-up', task['id']),
                            )
                            if cur.rowcount == 0:
                                # Row was already resolved by a concurrent cycle — nothing to do.
                                logger.debug("[Autonomy] Repair task id=%s already resolved; skipping duplicate update.", task['id'])
                        except _MySQLIntegrityError as dup_exc:
                            logger.warning(
                                "[Autonomy] Duplicate key ignored while resolving repair task id=%s: %s",
                                task['id'], dup_exc,
                            )
    ...
    async def ensure_bot_schema(self, cur, drone: str) -> None:
        if drone in self._bot_schema_ready:
            return
        # Aria should observe and repair the swarm, not run heavyweight DDL on every medic scan.
        # The music bots/SwarmPanel own these schemas; leave emergency migration opt-in only.
        if not self._bot_schema_autofix_enabled:
            self._bot_schema_ready.add(drone)
            if drone not in self._bot_schema_autofix_notice:
                logger.info(
                    "Skipping heavy bot schema DDL for %s; set ARIA_AUTONOMY_BOT_SCHEMA_AUTOFIX=1 to let Aria migrate bot schemas.",
                    drone,
                )
                self._bot_schema_autofix_notice.add(drone)
            return
        now = time.monotonic()
